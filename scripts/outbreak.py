...
def parse_feed_to_parts(feed):
    ...
    while i < len(string_array):
        ...
        covid_case = RegistredCovCase(patient_ordinal_number, covid19_suspected_at, covid19_confirmed_positive_at,
                                      covid19_confirmed_negative_at,
                                      patient_recovered_at, patient_deceased_at, patient_sex, patient_age, category,
                                      patient_addressOfStay_City, patient_addressOfStay_Street,
...
class RegistredCovCase:
    def __init__(self, patient_ordinal_number, covid19_suspected_at, covid19_confirmed_positive_at,
                 covid19_confirmed_negative_at, patient_recovered_at, patient_deceased_at,
                 patient_sex, patient_age, category, patient_addressOfStay_City, patient_addressOfStay_Street,
                 city_latitude, city_longitude, street_latitude, street_longitude,
                 note, is_public):
    ...
    def print_info(self):
        print("patient_ordinal_number: " + str(
            self.patient_ordinal_number) + " , " + "covid19_suspected_at: " + self.covid19_suspected_at + " , " + "covid19_confirmed_positive_at: " + self.covid19_confirmed_positive_at + " , " +
              ...
              "patient_sex: " + " , " + self.patient_sex + " , " + "patient_age: " + str(
            self.patient_age) + " , " + "category: " + self.category + " , " + "patient_adressOfStay_City: " + self.patient_addressOfStay_City + " , " +
              "patient_adressOfStay_Street: " + self.patient_addressOfStay_Street + " , " + "city_latitude: " + str(
            self.city_latitude) + " , " + "city_longitude: " + str(
            self.city_longitude) + " , " + "street_latitude: " + str(
            self.street_latitude) + " , " + "street_longitude: " + str(self.street_longitude) + " , " +
...
def load_pickle_matrix(file):
    ...
    for i in range(0,
                   len(matrix)):  # na riadku i a stlpci j je pocet obyvatelov, ktori sa presuvaju z mesta i do mesta j
...
class Square:  # EXTRA TODO
    ...
    def update(self, position, matrix, squares):
        ...
        self.Sus.append(int(
            self.Sus[len(self.Sus) - 1] - self.pismenobeta * self.Sus[len(self.Sus) - 1] * self.InfA[
                len(self.InfA) - 1] /
            (self.Sus[len(self.Sus) - 1] + self.InfA[len(self.InfA) - 1] + self.Rec[len(self.Rec) - 1]) - Bf_tmp))  #

        self.InfA.append(int(
            self.InfA[len(self.InfA) - 1] + self.pismenobeta * self.Sus[len(self.Sus) - 1] * (1 - self.coef_p) *
            self.InfA[
                len(self.InfA) - 1] /
        ...
        self.InfS.append(int(
            self.InfS[len(self.InfS) - 1] + self.pismenobeta * self.Sus[len(self.Sus) - 1] * self.coef_p * (
                        1 - self.coef_r) *
...
class Map:  # EXTRA TODO
    ...
    def color_square_stategy_max_ill(self):
        ...
        for x in self.squares:
            ...
            draw.rectangle([(x.upper_left[0], x.upper_left[1]), (x.lower_right[0], x.lower_right[1])],
                           fill=COLOR + (OPACITY,))
